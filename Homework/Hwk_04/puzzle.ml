...
module PuzzleF (S: SetS) : PuzzleS = struct
    ...
    let rec check_dict (word: string)(dict) : string list=
      if S.elem word dict then [word] else []
    ...
    let rec test (word: (char list * char list)) (dict) (ans)=
    match word with
      | (x1 , []) -> ans
      | (x1 , x2) -> match x2 with
          (*call test until x2 is empty making ans a list of tuples containing the word and changed word *)
          | [] -> []
          | x::xs -> test (x1@[x] , xs) dict (ans @ (check_dict  (UtilM.implode (x1@[String.get "j" 0]@xs)) dict)@
          (check_dict  (UtilM.implode (x1@[String.get "q" 0]@xs)) dict)@(check_dict  (UtilM.implode (x1@[String.get "x" 0]@xs)) dict)@
          (check_dict  (UtilM.implode (x1@[String.get "z" 0]@xs)) dict))
  ...
  let rec aws (word_list: string list) (answer) (dict) = 
    match word_list with 
  ...
  let answers (word_list: string list) =
