...
void row_sums_unroll4(matrix_t *matp, vector_t *sumsp){
  ...
  for(int i=0; i<mat.rows; i++){
    ...
    for(j=0; j<mat.cols-4; j+=4){
      sum0 += ((mat).data[((i)*((mat).cols)) + (j+0)]);
      sum1 += ((mat).data[((i)*((mat).cols)) + (j+1)]);
      sum2 += ((mat).data[((i)*((mat).cols)) + (j+2)]);
      sum3 += ((mat).data[((i)*((mat).cols)) + (j+3)]);
    ...
    for(; j<mat.cols; j+=1){
     ...
     ((mat).data[(i)] = (sum0+sum1+sum2+sum3));
  }
}
