...
int main(int argc, char **argv) {
  ...
  while (fgets(cmd, CMD_LEN, stdin) != NULL) {
    ...
    // Task 6: Wait for all background jobs
      ...
      // TODO Task 4: Set the child process as the target of signals sent to the
      // terminal via the keyboard. To do this, call 'tcsetpgrp(STDIN_FILENO,
      // <child_pid>)', where child_pid is the child's process ID just returned
      // by fork(). Do this in the parent process.

      // TODO Task 5: Handle the issue of foreground/background terminal process
      // groups. Do this by taking the following steps in the shell (parent)
      // process:
      ...
      // 1. Determine if the last token is "&". If present, use strvec_take() to
      // remove
