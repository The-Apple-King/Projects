...
/* As in C, the first character of an identifier is either a letter or
   an underscore. */
int isident_first_c(char c) { return isalpha_c(c) || c == '_'; }

/* After the first character, indentifiers can also contain digits. */
int isident_rest_c(char c) { return isalpha_c(c) || c == '_' || isdigit_c(c); }

/* Pre-declare structures that are pointed to by the global
   interpreter struct. The contents of these structures will be given
   later in the appropriate sections. */
struct var;  /* a variable */
struct stmt; /* a statement */

/* This structure is the root of all the mutable state that we
   maintain for the parsing and execution of a single program. At the
   moment we only ever execute one program, so these could have also
   been globals. But encapsulating them in a structure will make it
   easier to support multiple programs in the future. */
struct interp {
  ...
  /* Statements are kept in a singly-linked list sorted by line
     number: we use these links for normal control flow (other than
     goto), and to look up statements by their line number.  Note
     that though it is a good practice, it is not required that the
     statements appear in order in the file. */

  /* This pointer is the root of the linked list of statements. */
  struct stmt *first_stmt;
  ...
  /* Because the most common place to insert new statements into the
     ...
     position near the end of the list to accelerate insertions near
     the end. Specifically we keep a pointer to the next pointer of
     the last statement that came before the statement we most
     recently inserted. */
  struct stmt **last_prev_stmt;
};

/* Create a new, empty interpreter structure. A program starts with no
   variables and no statements. */
struct interp *new_interp(void) {
  struct interp *interp_p = xmalloc(sizeof(struct interp));
  interp_p->vars_head = 0;
  interp_p->first_stmt = 0;
  interp_p->last_prev_stmt = 0;
  return interp_p;
}
...
/* Variables in BCBASIC have a simple dynamic type system. They start
   ...
   operation that writes to a variable establishes it as either an
   integer or an array, and the subsequent operations have to
   match. */
enum var_type { UNINIT, INT, ARRAY };
...
struct var {
  /* This could almost be a const char *, since we don't modify
     the string except to free it. */
  char *name;
  struct var *next;
  enum var_type type;
  union {
...
/* Look up a variable based on its name. Note that the name pointer p
   ...
   exists. */
struct var *lookup_var(struct interp *interp_p, const char *p, size_t len) {
  struct var *vp;
  for (vp = interp_p->vars_head; vp; vp = vp->next) {
    if (strlen(vp->name) == len && memcmp(p, vp->name, len) == 0) return vp;
  }
  return 0;
}
...
struct var *intern_var(struct interp *interp_p, const char *p, size_t len) {
  struct var *vp = lookup_var(interp_p, p, len);
  if (vp) return vp;
  vp = xmalloc(sizeof(struct var));
  vp->name = xstrndup(p, len);
  vp->type = UNINIT;
  vp->next = interp_p->vars_head;
  interp_p->vars_head = vp;
  return vp;
}

/* Release the memory used for storage of variables, including the
   array storage for array variables. */
void free_vars(struct interp *interp_p) {
  struct var *next_var, *vp;
  for (vp = interp_p->vars_head; vp; vp = next_var) {
    free(vp->name);
    if (vp->type == ARRAY) free(vp->array_val);
    next_var = vp->next;
    free(vp);
  }
}
...
enum simple_type { INT_CONST, STR_CONST, VAR };
...
/* Parser for a variable. This follows an analogous calling convention
   as scan_int. */
const char *scan_var(struct interp *interp_p, const char *p,
                     struct simple *res) {
  const char *start = p;
  if (!isident_first_c(*p)) return 0;
  p++;
  while (isident_rest_c(*p)) p++;
  res->type = VAR;
  res->var_ref = intern_var(interp_p, start, p - start);
  return p;
}
...
*/
int scan_line(struct interp *interp_p, const char *line, const char *format,
              struct simple *res, int *err_pos) {
  const char *p = line;
  const char *f;
  int j = 0;
  for (f = format; *f; f++) {
...
enum binop_type { PLUS, MINUS, TIMES, DIVIDE, MOD, POW };

/* BCBASIC supports all 6 standard ways of comparing two integers: */
enum compare_type { LT, GT, LE, GE, EQ, NE };
...
void insert_stmt(struct interp *ip, struct stmt *new_sp) {
  struct stmt **prev;
  struct stmt *sp;
  if (ip->last_prev_stmt &&
      (*ip->last_prev_stmt)->line_num < new_sp->line_num) {
    /* If the new statement would go after last_prev_stmt, start
       from there */
    prev = ip->last_prev_stmt;
    sp = *prev;
  } else {
...
struct stmt *lookup_stmt(struct interp *interp_p, uint64_t line) {
  struct stmt *sp;
  for (sp = interp_p->first_stmt; sp; sp = sp->next) {
    if (sp->line_num == line) {
      return sp;
...
/* Free the memory used by all of the statements in a program. Though
   statements are a complex-looking structure, most of their contents
   are inlined in the structure, or separately owned for deallocation
   purposes. For instance we don't follow the pointers to variables or
   target statements when when freeing. The only separate allocation
   owned by the statement that needs to be freed is string constants
   in print statements. */
void free_stmts(struct interp *interp_p) {
  struct stmt *next_stmt, *sp;
  for (sp = interp_p->first_stmt; sp; sp = next_stmt) {
    if ((sp->type == PRINT || sp->type == PRINTLN) &&
        sp->arg1.type == STR_CONST) {
      free(sp->arg1.str_const);
    }
    next_stmt = sp->next;
    free(sp);
  }
...
/* This is 10**19-1, where 10**19 is the largest power of ten that
   fits in a uint64_t. */
#define MAX_LINE_NUM 9999999999999999999UL

/* Parse one line of the input file into a statement, and return it;
   or return NULL for a comment line. Syntax errors are fatal. */
struct stmt *parse_line(struct interp *interp_p, const char *p,
                        uint64_t *next_line_num_ptr) {
  uint64_t line_num;
  struct stmt *sp;
  struct simple res[4];
  while (*p == ' ' || *p == '\t') p++;
  ...
  if (*p == '#' ||
    ...
    /* A line that is blank or consisting only of spaces and tabs
       is also ignored like a comment. */
    return 0;
  }
  ...
  if (isdigit_c(*p)) {
    ...
    line_num = *next_line_num_ptr;
  }

  if (line_num > MAX_LINE_NUM) {
    fprintf(stderr, "Line number %lu too large\n", line_num);
    exit(1);
  }

  if (*p == '#' || !*p ||
      (p[0] == 'r' && p[1] == 'e' && p[2] == 'm' && !isalpha_c(p[3]))) {
    /* A numbered blank or comment line doesn't create a
       statement, but it still affects the next line number. */
    *next_line_num_ptr = line_num + 1;
    return 0;
  }
  ...
  /* To parse a statement, we attempt to match it against patterns
     ...
     digits in them. */
  if (scan_line(interp_p, p, "%v_=_%e", res, &err_pos)) {
    sp->type = LET;
    sp->lhs = res[0].var_ref;
    sp->arg1 = res[1];
  } else if (scan_line(interp_p, p, "print0 %s", res, &err_pos) ||
             scan_line(interp_p, p, "print0 %e", res, &err_pos) ||
             scan_line(interp_p, p, "println1 %s", res, &err_pos) ||
             scan_line(interp_p, p, "println1 %e", res, &err_pos)) {
    ...
    sp->arg1 = res[1];
  } else if (scan_line(interp_p, p, "%v_=_%e_+0_%e", res, &err_pos) ||
             scan_line(interp_p, p, "%v_=_%e_-1_%e", res, &err_pos) ||
             scan_line(interp_p, p, "%v_=_%e_*2_%e", res, &err_pos) ||
             scan_line(interp_p, p, "%v_=_%e_/3_%e", res, &err_pos) ||
             scan_line(interp_p, p, "%v_=_%e mod4 %e", res, &err_pos) ||
             scan_line(interp_p, p, "%v_=_%e_**5_%e", res, &err_pos)) {
    ...
    sp->arg2 = res[3];
  } else if (scan_line(interp_p, p, "%v_=_%v_[_%e_]", res, &err_pos)) {
    sp->type = ALOAD;
    sp->lhs = res[0].var_ref;
    sp->array_var = res[1].var_ref;
    sp->arg1 = res[2];
  } else if (scan_line(interp_p, p, "%v_[_%e_]_=_%e", res, &err_pos)) {
    sp->type = ASTORE;
    sp->array_var = res[0].var_ref;
    sp->arg1 = res[1];
    sp->arg2 = res[2];
  } else if (scan_line(interp_p, p, "clear %v", res, &err_pos)) {
    sp->type = CLEAR;
    sp->array_var = res[0].var_ref;
  } else if (scan_line(interp_p, p, "%v_=_length %v", res, &err_pos)) {
    sp->type = LENGTH;
    sp->lhs = res[0].var_ref;
    sp->array_var = res[1].var_ref;
  } else if (scan_line(interp_p, p, "goto %d", res, &err_pos)) {
    sp->type = GOTO;
    sp->targ_line = res[0].int_const;
  } else if (scan_line(interp_p, p, "if %e_<0_%e then goto %d", res,
                       &err_pos) ||
             scan_line(interp_p, p, "if %e_<=1_%e then goto %d", res,
                       &err_pos) ||
             scan_line(interp_p, p, "if %e_>2_%e then goto %d", res,
                       &err_pos) ||
             scan_line(interp_p, p, "if %e_>=3_%e then goto %d", res,
                       &err_pos) ||
             scan_line(interp_p, p, "if %e_==4_%e then goto %d", res,
                       &err_pos) ||
             scan_line(interp_p, p, "if %e_<>5_%e then goto %d", res,
                       &err_pos)) {
    ...
    fprintf(stderr, "%s\n", p);
    for (i = 0; i < err_pos; i++) fprintf(stderr, " ");
    fprintf(stderr, "^\n");
    exit(1);
  }
  insert_stmt(interp_p, sp);
  *next_line_num_ptr = line_num + 1;
  return sp;
}
...
void parse_program(struct interp *interp_p, char *fname) {
  char *buf;
  char *line, *p;
  slurp_file(fname, &buf);
  line = buf;

  /* If lines don't have an explicit line number, we assign line
     numbers sequentually: this variable repesents the next one in
     sequence. */
  ...
  for (p = buf; *p; p++) {
    if (*p == '\n') {
      *p = 0;
      parse_line(interp_p, line, &next_line_num);
      line = p + 1;
    }
  }
  free(buf);
...
void list_program(struct interp *interp_p) {
  struct stmt *sp;
  for (sp = interp_p->first_stmt; sp; sp = sp->next) {
...
void resolve_jumps(struct interp *interp_p) {
  struct stmt *sp;
  for (sp = interp_p->first_stmt; sp; sp = sp->next) {
    if (sp->type == GOTO || sp->type == IFGOTO) {
      struct stmt *targ_sp = lookup_stmt(interp_p, sp->targ_line);
      if (!targ_sp) {
        fprintf(stderr,
                "Missing taget line %lu "
                "for goto on line %lu\n",
...
int64_t do_modulo(int64_t arg1, int64_t arg2) {
  if (arg2 == 0) fatal("Modulo by zero");
  int64_t rem = arg1 % arg2;
  if (arg1 == 0) return 0;
  if (rem == 0) return 0;
  if (arg1 < 0 && arg2 > 0) return rem + arg2;
  if (arg1 > 0 && arg2 < 0) return rem - arg2;
  return rem;
}

/* Exponentiation ("**") is implemented for integers with a classic
   square-and-multiply algorithm, and has explicit overflow
   checking. */
int64_t do_pow(int64_t arg1, int64_t arg2) {
  if (arg2 == 0) return 1;
  if (arg1 == 1) return 1;
  if (arg2 == 1) return arg1;
  /* This particular value matches what it would overflow to, so the
     result would be the same if we used the overflow case
     below. But this value is exact. */
  if (arg1 == -2 && arg2 == 63) return 0x8000000000000000;
  int64_t sign = 1;
  int64_t base = arg1;
  int64_t exp = arg2;
  if (base < 0) {
  ...
  if (exp < 0) {
    if (base > 1)
      ...
      fatal("Zero raised to negative exponent");
  }
  if ((base == 2 && exp >= 63) || (base == 3 && exp > 39) ||
      (base == 4 && exp > 31) || (base == 5 && exp > 27) ||
      (base == 6 && exp > 24) || (base == 7 && exp > 22) ||
      (base == 8 && exp >= 21) || (base == 9 && exp > 19) ||
      (base >= 10 && exp > 18) || (base >= 12 && exp > 17) ||
      (base >= 14 && exp > 16) || (base >= 16 && exp > 15) ||
      (base >= 19 && exp > 14) || (base >= 23 && exp > 13) ||
      (base >= 29 && exp > 12) || (base > 38 && exp >= 12) ||
      (base > 52 && exp >= 11) || (base > 78 && exp >= 10) ||
      (base >= 128 && exp >= 9) || (base > 234 && exp >= 8) ||
      (base >= 512 && exp >= 7) || (base > 1448 && exp >= 6) ||
      (base > 6208 && exp >= 5) || (base > 55108 && exp >= 4) ||
      (base >= 2097152 && exp >= 3) || (base > 3037000499 && exp >= 2))
    return sign == 1 ? 0x7fffffffffffffff : 0x8000000000000000;
  int64_t prod = 1;
...
int64_t do_binop(enum binop_type b_type, int64_t arg1, int64_t arg2) {
  switch (b_type) {
    ...
    case TIMES:
      return arg1 * arg2;
    case DIVIDE: {
      if (arg2 == 0) fatal("Divide by zero");
      if (arg1 == 0x8000000000000000 && arg2 == -1)
        fatal("Signed division overflow");
      return arg1 / arg2;
    }
    case MOD:
      return do_modulo(arg1, arg2);
    case POW:
      return do_pow(arg1, arg2);
  }
  assert(0);
}
...
int64_t array_load(struct var *avar, int64_t index) {
  ...
  if ((index >= 0 && (size_t)index >= avar->allocated) || index < -avar->length)
    return 0;
  if (index < 0) index += avar->length;
...
void array_grow(struct var *avar, int64_t new_len) {
  if (new_len <= 0) fatal("Array size overflow");
...
void array_store(struct var *avar, int64_t index, int64_t val) {
  ...
  if (index < -avar->length) {
    ...
    if (index >= 0x7fffffffffffffff) fatal("Index overflow in store");
...
void array_copy(struct var *lhs, struct var *rhs) {
  ...
  if (lhs == rhs) {
    /* Copying an array to itself is a no-op */
    return;
  }
  ...
  if (new_alloc <= 0 || new_alloc >= (1L << 60)) fatal("Array copy overflow");
...
struct stmt *exec_goto(struct stmt *sp) { return sp->targ_stmt; }
...
/* The main execution loop of BCBASIC calls the opcode for statements
   ...
   program counter. */
void exec_program(struct interp *interp_p) {
  struct stmt *sp = interp_p->first_stmt;
  while (sp) {
...
void free_interp(struct interp *interp_p) {
  free_stmts(interp_p);
  free_vars(interp_p);
  free(interp_p);
}
...
int main(int argc, char **argv) {
  ...
  char *fname = argv[1];
  struct interp *interp_p = new_interp();
  parse_program(interp_p, fname);
  list_program(interp_p);
  resolve_jumps(interp_p);
  exec_program(interp_p);
  free_interp(interp_p);
  return 0;
}
